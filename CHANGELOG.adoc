...
* Fix the `document` field/`getDocument()`/`getConverter()` JSDoc on `AbstractNode` (and the generated `.d.ts` for `AbstractNode` and `Table.Cell`) referring to the ambient DOM `Document` type instead of Asciidoctor's own `Document` class from `document.js`. TypeScript resolved the unqualified `Document` name to `lib.dom.d.ts`, so `node.getDocument()` was typed as the browser DOM document rather than the Asciidoctor document, silently defeating type checking on any code that called Asciidoctor `Document` methods on the result
